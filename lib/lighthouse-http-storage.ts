// HTTP-based Pinata storage to avoid SDK dependencies
// This uses direct HTTP calls to Pinata API instead of the SDK

import { getLighthouseHash, updateLighthouseHash } from './storage-config';

// Pinata API key
const PINATA_API_KEY = process.env.PINATA_JWT || process.env.NEXT_PUBLIC_PINATA_JWT;
...
async function getStorageData(): Promise<StorageData> {
  try {
    ...
    const response = await fetch(`https://gateway.pinata.cloud/ipfs/${storageHash}`);
    if (!response.ok) {
...
async function saveStorageData(data: StorageData): Promise<string> {
  try {
    ...
    if (result && result.IpfsHash) {
      ...
      throw new Error('Failed to upload storage data to Pinata');
    }
  } catch (error) {
...
export async function storeAdPlacement(
  ...
  const placement: StoredPlacement = {
    ...
    placementId,
    advertiserWallet,
    contentUrl: `https://gateway.pinata.cloud/ipfs/${contentHash}`,
    contentHash,
    price,
    currency,
    durationMinutes,
...
export async function getAdPlacement(slotId: string): Promise<StoredPlacement | null> {
  ...
  // Check if ad has started (now >= startsAt)
  if (now < startsAt) {
    console.log(`Ad for slot ${slotId} has not started yet. Starts at: ${startsAt.toISOString()}, Current time: ${now.toISOString()}`);
    return null; // Ad hasn't started yet, don't show it
  }

  // Check if ad has expired (now > expiresAt)
  if (now > expiresAt) {
    ...
    if (queue && queue.queue.length > 0) {
      ...
      // Return the newly activated placement
  ...
  // Ad is active (now >= startsAt && now <= expiresAt)
  return placement;
}
