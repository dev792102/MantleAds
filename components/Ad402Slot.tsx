...
export const Ad402Slot: React.FC<Ad402SlotProps> = ({
  ...
  const { websiteId } = useAd402Context();
  ...
  const fetchAdContent = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`Failed to fetch ad for slot ${slotId}:`, response.status, response.statusText);
        setHasAd(false);
        setAdContent(null);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Poll for ad updates every 10 minutes (users can refresh manually for immediate updates)
...
export interface PaymentSignatureData {
  ...
  userAddress: string;
  paymentInfo: any;
  mntConfig: USDCConfig;
  chainId: number;
}

// MNT contract configurations (using wrapped MNT for ERC20 compatibility)
export const MNT_CONTRACTS = {
  "5000": {
    address: "0x78c1b0c915c4faa5fffa6cabf0219da63d7f4cb8", // Wrapped MNT (WMNT)
    name: "Wrapped Mantle",
    decimals: 18,
    version: "1"
  }
...
export class MNTService {
  /**
   * Get MNT configuration for a specific chain
   */
  static getMNTConfig(chainId: number): USDCConfig | null {
    const config = MNT_CONTRACTS[chainId.toString() as keyof typeof MNT_CONTRACTS];
    return config || null;
  }
  ...
  static isSupportedChain(chainId: number): boolean {
    return chainId === 5000; // Mantle Mainnet
  }
  ...
  static async generateEIP3009Signature(
    ...
    const mntConfig = this.getMNTConfig(chainId);
    
    if (!mntConfig) {
      throw new Error(`Unsupported network. Chain ID: ${chainId}`);
    }

    // EIP-712 domain with correct MNT details
    const domain = {
      name: mntConfig.name,
      version: mntConfig.version,
      chainId,
      verifyingContract: mntConfig.address
    };
    ...
    // Convert amount to wei (18 decimals for MNT)
    const value = (parseFloat(amount) * Math.pow(10, mntConfig.decimals)).toString();
